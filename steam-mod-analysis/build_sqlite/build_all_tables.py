...
步骤：
  ...
  3. mod_subscriber_exposure_ratio
  4. mod_subscriber_exposure_ratio_with_mod_name
  5. mod_tag_flags
  6. 合并 browse 表
  7. authors（来源 mod_browse_info.author）
  8. metadata
  9. statistic
  10. 导出 Excel
"""
...
from validate_build_inputs import fail, validate_build_inputs  # noqa: E402
PRIVATE_FLAGS_TABLE = "mod_private_like_flags"
RATIO_TABLE = "mod_subscriber_exposure_ratio"
...
# ---------------------------------------------------------------------------
# mod_subscriber_exposure_ratio（原 build_subscriber_exposure_ratio.py）
# ---------------------------------------------------------------------------
...
def create_ratio_table(conn: sqlite3.Connection, table_name: str) -> None:
    conn.execute(
        f"""
        CREATE TABLE IF NOT EXISTS "{table_name}" (
            mod_id TEXT PRIMARY KEY,
...
def rebuild_subscriber_exposure_ratio(
    db_path: Path,
    output_table: str = RATIO_TABLE,
    ...
    with sqlite3.connect(db_path) as conn:
        if not _table_exists(conn, "mods"):
            raise RuntimeError('Required table "mods" is missing in the database.')
        ...
        create_ratio_table(conn, output_table)

        conn.execute(f'DELETE FROM "{output_table}";')
        conn.execute(
            f"""
            INSERT INTO "{output_table}" (mod_id, subscriber_exposure_ratio)
            SELECT
                m.mod_id,
...
def rebuild_subscriber_exposure_ratio_with_mod_name(
    ...
    with sqlite3.connect(db_path) as conn:
        if not _table_exists(conn, "mods"):
            raise RuntimeError('Required table "mods" is missing in the database.')
        if not _table_exists(conn, ratio_table):
            raise RuntimeError(
                f'Ratio table "{ratio_table}" not found. Run subscriber exposure ratio step first.'
            )
        ...
        conn.execute(
            ...
            FROM "{ratio_table}" AS r
            INNER JOIN mods AS m ON m.mod_id = r.mod_id
            ORDER BY r.subscriber_exposure_ratio DESC, r.mod_id;
            """
        )
        conn.commit()
        inserted = conn.execute(f'SELECT COUNT(*) FROM "{output_table}";').fetchone()[0]
...
def main() -> None:
    ...
    print("== 0/10 检查依赖 ==")
    html_count = validate_build_inputs(args.mods_dir, args.browse_db_path)
    ...
    print("== 2/10 mod_private_like_flags ==")
    total_flags, private_count = rebuild_flags(args.db_path)
    print(f"  rows: {total_flags}, private-like: {private_count}")

    print("== 3/10 mod_subscriber_exposure_ratio ==")
    ratio_rows, ratio_gt_one = rebuild_subscriber_exposure_ratio(args.db_path)
    print(f"  rows: {ratio_rows}, ratio > 1: {ratio_gt_one}")

    print(f"== 4/10 {RATIO_WITH_NAME_TABLE} ==")
    ratio_name_rows = rebuild_subscriber_exposure_ratio_with_mod_name(args.db_path)
    print(f"  rows: {ratio_name_rows}")

    print(f"== 5/10 {TAG_FLAGS_TABLE} ==")
    tag_rows, tag_skipped = rebuild_tag_flags(args.mods_dir, args.db_path)
    print(f"  rows: {tag_rows}, skipped html: {tag_skipped}")

    print("== 6/10 merge browse tables ==")
    with sqlite3.connect(args.db_path) as conn:
        browse_tables = merge_browse_tables(conn, args.browse_db_path)
    print(f"  tables: {', '.join(browse_tables)}")

    print(f"== 7/10 {AUTHORS_TABLE} ==")
    with sqlite3.connect(args.db_path) as conn:
        author_count, chinese_count = rebuild_authors_table(conn)
    print(f"  authors: {author_count}, is_chinese=1: {chinese_count}")

    print(f"== 8/10 {METADATA_TABLE} ==")
    with sqlite3.connect(args.db_path) as conn:
        ...
        gap_count = conn.execute(
            ...
            LEFT JOIN mods AS m ON m.mod_id = b.mod_id
            WHERE m.mod_id IS NULL;
            """
        ).fetchone()[0]
    print(f"  mod_count: {mod_count}, browse_only: {gap_count}")

    print("== 9/10 statistic ==")
    with sqlite3.connect(args.db_path) as conn:
        stat_rows = create_statistic_table(conn)
    print(f"  rows: {stat_rows}")

    if args.skip_excel:
        print("== 10/10 Excel export (skipped) ==")
        print(f"SQLite: {args.db_path}")
        return

    print("== 10/10 Excel export ==")
    export_database(args.db_path, args.output_file, main_table="mods")

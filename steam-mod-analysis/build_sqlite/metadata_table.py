...
def compute_browse_without_detail(conn: sqlite3.Connection) -> list[str]:
    if not _table_exists(conn, BROWSE_INFO_TABLE) or not _table_exists(conn, "mods"):
        return []
    rows = conn.execute(
        f"""
        SELECT b.mod_id
...
def _detail_html_path(html_root: Path, row_id: int, *, use_mod_html_buckets: bool = True) -> Path:
    """与 resumable-batch-fetch fetch_via_id.html_path 一致。"""
    if use_mod_html_buckets:
        return html_root / str(row_id)[:2] / f"{row_id}.html"
...
def _created_time_from_detail_html(
    ...
    *,
    ...
    """取 row_id 1..9 中第一个存在的详情页 HTML 文件时间。"""

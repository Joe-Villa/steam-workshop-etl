...
_ANALYSIS_SRC = Path(__file__).resolve().parent.parent / "analysis_src"
if str(_ANALYSIS_SRC) not in sys.path:
    sys.path.insert(0, str(_ANALYSIS_SRC))

from generate_domestic_reports import calc_stats, percentile  # noqa: E402
...
def _fetch_all_subscribers(conn: sqlite3.Connection) -> list[int]:
    rows = conn.execute(
        "SELECT subscribers FROM mods ORDER BY subscribers DESC, mod_id ASC;"
    ).fetchall()
...
def _fetch_ratio_rows(conn: sqlite3.Connection) -> list[tuple[float, int]]:
    rows = conn.execute(
        """
        SELECT r.subscriber_exposure_ratio, m.subscribers
        FROM mod_subscriber_exposure_ratio AS r

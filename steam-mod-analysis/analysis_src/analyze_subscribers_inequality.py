...
def load_data(db_path: Path, filter_private_like: bool = True) -> tuple[np.ndarray, np.ndarray]:
    query = """
    SELECT
...
def write_markdown_report(
    ...
    lines.append(f"- 数据源：`{db_path_label}`，字段 `mods.subscribers`。")
    lines.append(
        "- 样本口径：仅保留非自用模组（`mod_private_like_flags.is_private_like = 0`）。"
        if filter_private_like
        else "- 样本口径：全量模组（未过滤自用标记）。"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include mods flagged as private-like. Default excludes them.",
    )
    ...
    filter_private_like = not args.include_private_like
    subscribers, groups = load_data(db_path, filter_private_like=filter_private_like)
    ...
    write_markdown_report(
        ...
        subscribers,
        groups,
        filter_private_like=filter_private_like,
        game=profile,

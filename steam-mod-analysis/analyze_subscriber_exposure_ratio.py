...
DEFAULT_TABLE = "mod_subscriber_exposure_ratio_with_mod_name"
...
def load_mod_ratio_rows(db_path: Path, table_name: str) -> list[ModRatioRow]:
    ...
    if "mod_name" in columns:
        sql = f"""
            SELECT r.mod_id, r.mod_name, r.subscribers, r.subscriber_exposure_ratio
            FROM "{table_name}" AS r
            {join_private_like_flags("r")}
            WHERE {non_private}
            ORDER BY r.mod_id;
        """
    else:
        sql = f"""
            ...
            FROM "{table_name}" AS r
            INNER JOIN "{MODS_TABLE}" AS m ON m.mod_id = r.mod_id
            {flags_join}
            WHERE {non_private}
            ORDER BY r.mod_id;
        """
...
def write_report(
    ...
    lines.append(
        f"- 数据源：`{db_path_label}`，表 `mod_subscriber_exposure_ratio_with_mod_name`。"
    )
    lines.append("- 数据为模组维度汇总：每个模组有当前订阅量、Unique Visitors，以及二者之比（订阅 / 曝光）。")
    from sample_scope import SAMPLE_SCOPE_NOTE

    lines.append(f"- {SAMPLE_SCOPE_NOTE} 比率表另排除 `exposure = 0`。")
    ...
    lines.append("")
    lines.append("分组以 `aaa_mods.subscribers`（当前订阅量）为准，边界为左闭右开区间，末组为闭区间：")
    lines.append("")
...
def main() -> None:
    ...
    from data_paths import MODS_TABLE, ensure_data_dirs
    ...
    if not all_rows:
        raise RuntimeError(f"No rows in table {args.table!r} (after join with {MODS_TABLE}).")
